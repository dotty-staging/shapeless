...
object K0 {
  ...
  given Object as Ops { // TODO: We probably don't need the given anymore.
  ...
  inline given [F[_], T] => (gen: Generic[T]) => Instances[F, T] as mkInstances =
    inline gen match {
      case p: ProductGeneric[T]   => mkProductInstances[F, T](using p)
      case c: CoproductGeneric[T] => mkCoproductInstances[F, T](using c)
    }

  inline given [F[_], T] => (gen: ProductGeneric[T]) => ProductInstances[F, T] as mkProductInstances =
    ErasedProductInstances[K0.type, F[T], LiftP[F, gen.MirroredElemTypes]](gen)

  inline given [F[_], T] => (gen: CoproductGeneric[T]) => CoproductInstances[F, T] as mkCoproductInstances =
...
object K1 {
  ...
  inline given [F[_[_]], T[_]] => (gen: Generic[T]) => Instances[F, T] as mkInstances =
    inline gen match {
      case p: ProductGeneric[T] => mkProductInstances[F, T](using p)
      case c: CoproductGeneric[T] => mkCoproductInstances[F, T](using c)
    }

  inline given [F[_[_]], T[_]] => (gen: ProductGeneric[T]) => ProductInstances[F, T] as mkProductInstances =
    ErasedProductInstances[K1.type, F[T], LiftP[F, gen.MirroredElemTypes]](gen)

  inline given [F[_[_]], T[_]] => (gen: CoproductGeneric[T]) => CoproductInstances[F, T] as mkCoproductInstances =
...
object K11 {
  ...
  inline given [F[_[_[_]]], T[_[_]]] => (gen: Generic[T]) => Instances[F, T] as mkInstances =
    inline gen match {
      case p: ProductGeneric[T]   => mkProductInstances[F, T](using p)
      case c: CoproductGeneric[T] => mkCoproductInstances[F, T](using c)
    }

  inline given [F[_[_[_]]], T[_[_]]] => (gen: ProductGeneric[T]) => ProductInstances[F, T] as mkProductInstances =
    ErasedProductInstances[K11.type, F[T], LiftP[F, gen.MirroredElemTypes]](gen)

  inline given [F[_[_[_]]], T[_[_]]] => (gen: CoproductGeneric[T]) => CoproductInstances[F, T] as mkCoproductInstances =
