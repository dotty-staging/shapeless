...
object Monoid {
  ...
  given [A] => (inst: K0.ProductInstances[Monoid, A]) => Monoid[A] as monoidGen {
...
object Eq {
  ...
  given [A] => (inst: K0.ProductInstances[Eq, A]) => Eq[A] as eqGen {
    def eqv(x: A, y: A): Boolean = inst.foldLeft2(x, y)(true: Boolean)(
      [t] => (acc: Boolean, eqt: Eq[t], t0: t, t1: t) => Complete(!eqt.eqv(t0, t1))(false)(true)
    )
  }

  given [A] => (inst: => K0.CoproductInstances[Eq, A]) => Eq[A] as eqGenC {
...
object Functor {
  ...
  given [F[_], G[_]] => (ff: Functor[F], fg: Functor[G]) => Functor[[t] =>> F[G[t]]] {
    def map[A, B](fga: F[G[A]])(f: A => B): F[G[B]] = ff.map(fga)(ga => fg.map(ga)(f))
  }

  given [F[_]] => (inst: => K1.Instances[Functor, F]) => Functor[F] as functorGen {
...
object FunctorK {
  ...
  given [H[_[_]]] => (inst: => K11.Instances[FunctorK, H]) => FunctorK[H] as functorKGen {
...
object Bifunctor {
  ...
  given [F[_, _]] => (inst: => K2.Instances[Bifunctor, F]) => Bifunctor[F] as bifunctorGen {
    def bimap[A, B, C, D](fab: F[A, B])(f: A => C, g: B => D): F[C, D] =
...
object Data extends Data0 {
  ...
  given [F, T, R] => (inst: K0.ProductInstances[DFR[F, R], T]) => Data[F, T, R] as dataGen =
  ...
  given [F, T, R] => (inst: => K0.CoproductInstances[DFR[F, R], T]) => Data[F, T, R] as dataGenC =
...
object DataT {
  ...
  given [F, T] => (inst: => K0.Instances[DF[F], T]) => Aux[F, T, T] as dataTGen =
...
object Empty {
  ...
  given [A] => (inst: K0.ProductInstances[Empty, A]) => Empty[A] as emptyGen =
    mkEmpty(inst.construct([a] => (ma: Empty[a]) => ma.empty))

  inline given [A] => (gen: K0.CoproductGeneric[A]) => Empty[A] as emptyGenC =
...
object EmptyK {
  ...
  given [C] => (ec: Empty[C]) => EmptyK[Const[C]] as emptyK {
  ...
  given [A[_]] => (inst: K1.ProductInstances[EmptyK, A]) => EmptyK[A] as emptyKGen =
    mkEmptyK([t] => () => inst.construct([f[_]] => (ef: EmptyK[f]) => ef.empty[t]))

  inline given [A[_]] => (gen: K1.CoproductGeneric[A]) => EmptyK[A] as emptyKGenC =
...
object Alt1 {
  ...
  inline given [F[_[_]], G[_[_]], T[_]] => Alt1[F, G, T] as apply = summonFrom {
...
object Pure {
  ...
  given [A[_]] => (inst: K1.ProductInstances[Alt1.Of[Pure, EmptyK], A]) => Pure[A] as pureGen =
    mkPure[A]([t] => (a: t) => inst.construct([f[_]] => (af: Alt1.Of[Pure, EmptyK][f]) => af.fold[f[t]](_.pure(a))(_.empty[t])))

  inline given [A[_]] => (gen: K1.CoproductGeneric[A]) => Pure[A] as pureGenC =
...
object Show {
  ...
  given [T] => (inst: K0.ProductInstances[Show, T], labelling: Labelling[T]) => Show[T] as showGen {
    def show(t: T): String = {
  ...
  given [T] => (inst: => K0.CoproductInstances[Show, T]) => Show[T] as showGenC {
...
object Read {
  ...
  given [T] => (inst: K0.ProductInstances[Read, T], labelling: Labelling[T]) => Read[T] as readGen {
    def read(s: String): Option[(T, String)] = {
  ...
  given [T] => (inst: => K0.CoproductInstances[Read, T], labelling: Labelling[T]) => Read[T] as readGenC {
    def read(s: String): Option[(T, String)] = {
